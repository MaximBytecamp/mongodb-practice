...
### Способ 1 — одинаково на любой системе

Скрипт снимает план и кладёт его в файл. Кавычек в команде нет, запрос и
запись файла идут внутри контейнера, поэтому команда работает одинаково в
Windows, macOS и Linux и в любой оболочке — PowerShell, `cmd.exe`, Терминал,
Git Bash.
...
Первое число — номер запроса: `1` — запрос про ошибки платежей, `2` — запрос
из задачи 4. Второе слово — имя файла. Файл ложится в
`work/praktikum-explain/`, то есть сразу туда, где собирается отчёт.

```
Запрос 1 — ошибки payments дольше 500 мс, свежие сверху
  файл:                  work/praktikum-explain/plan-1.json
  возвращено:            33
...
коллекции, которое вы сделали в оболочке. Последняя строка вывода показывает
текущие индексы — по ней видно, если от предыдущей задачи остался лишний.

### Способ 2 — командой mongosh

Работает в PowerShell, Терминале macOS и Linux. В `cmd.exe` не работает:
там одинарные кавычки не группируют текст в один аргумент, и `mongosh`
получает обрывок запроса.
