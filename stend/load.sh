...
# в справочнике и в ответах к практикам совпадут.
...
if command -v mongoimport >/dev/null 2>&1; then
  import() { mongoimport --uri "$URI" --db "$1" --collection "$2" --file "$3" --jsonArray --drop --quiet; }
  ...
  import() { docker exec -i "$CONTAINER" mongoimport --db "$1" --collection "$2" --jsonArray --drop --quiet < "$3"; }
else
